...
{
    unsigned int ret = 0;
    unsigned short str_start_flag = 0;
    unsigned short cnt_0xff = 0;
    char arr[32];
    char *temp = arr;
    unsigned short c = 0;
    long start;
...
void sendCommand(char *command)
{
    ...
    nexSerial_write(0xFF);
    nexSerial_write(0xFF);
	/*uartWriteString(UART_GPIO,command);
	uartWriteByte(UART_GPIO,0xFF);
	uartWriteByte(UART_GPIO,0xFF);
	uartWriteByte(UART_GPIO,0xFF);*/
}
...
{
    static unsigned short __buffer[10];
    ...
    while (nexSerial_available() == true)
    {
        nexDelay(10);
        c = nexSerial_read();
        ...
        if (NEX_RET_EVENT_TOUCH_HEAD == c)
        {
            ...
            //{
                ...
                {
                    NexTouch_iterate(nex_listen_list, __buffer[1], __buffer[2], (long)__buffer[3]);
                }
            //}
        }
    }
